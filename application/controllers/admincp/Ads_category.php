...
class Ads_category extends MY_Controller {
    ...
    public function detail($id = 0) {
        ...
        $this->data['category'] = $this->category_item->get_list($input);
        ...
        $this->data['title'] = 'Thêm danh mục ADS';
    ...
    public function clean_trash() {
        ...
        if ($object_id_del) {
            ...
            if ($check_del_product) {
                ...
                foreach($check_del_product as $row) {
                    //clear image
                    @unlink($path_upload . $row->image_link);      
                }
                $this->item->del_rule('cid in (' . $object_id_del . ')');
            } else {
